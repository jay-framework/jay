...
function BoardConstructor({ title }: Props<BoardProps>, refs: BoardElementRefs) {
    ...
    function moveTask(pillarId: string, task: PillarTask, pillarOffset: number, taskOffset: number) {
        let pillarIndex = pillars().findIndex(pillar => pillar.pillarId === pillarId);
        let taskIndex = pillars()[pillarIndex].pillarData.tasks.findIndex(aTask => aTask.id === task.id)
        let newTaskIndex = Math.min(taskIndex + taskOffset, pillars()[pillarIndex + pillarOffset].pillarData.tasks.length);
        ...
        let jsonPatch: JSONPatch = [
            ...
            {op: ADD, path: [pillarIndex + pillarOffset, "pillarData", "tasks", newTaskIndex], value: task}
        ];
