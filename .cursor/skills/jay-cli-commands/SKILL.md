...
1. **Config templates** — If no config file exists for a plugin, creates one with placeholder values (e.g., `config/.wix.yaml` with API key placeholders)
2. **Credential validation** — Attempts to initialize plugin services, reports success or failure
