...
const [todos, setTodos] = createState(
  mutableObject(initialTodos().map((_) => ({ ..._, isEditing: false, editText: '' }))),
);
```
...
The main idea of supporting _unchanged optimization_ is to identify when an item has not change and has not moved - which
...
The algorithm makes the assumption that _small_ changes can be found fast and require small number of mutations to describe.
