...
3. support for 3 rendering modes 4. slow rendering 5. fast rendering 6. fast and interactive rendering
 ...
 */
type ValidateRenderingMode<Mode extends string> = Mode extends 'slow' | 'fast' | 'fast+interactive'
  ? Mode
  : never;
```
...
export interface SchemaValidationRules {
   ...
   */
  schemaMatchesViewState: (viewStateKeys: string[], schemaKeys: string[]) => boolean;
}
```
...
type SlowViewState = NarrowViewStateByPhase<ProductPageViewState, typeof productPageSchema, 'slow'>;
// Result: {
//   name: string;
//   sku: string;
//   price: number;
...
type FastViewState = NarrowViewStateByPhase<ProductPageViewState, typeof productPageSchema, 'fast'>;
// Result: {
//   inStock: boolean;
//   quantity: number;
...
export function object<ViewState extends object>(properties: RenderingManifestSchema<ViewState>) {
  return {
    type: 'object' as const,
    properties,
  };
