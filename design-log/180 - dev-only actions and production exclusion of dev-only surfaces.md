...
- **DL#179** — runtime (`.`, compiler-free) vs tools (`./tools`, compiler-OK) entry split. Actions are
  the *serving* primitive (`.`, compiler-free); commands are the *tools* primitive (`./tools`).
...
1. **Settings pages need actions, not commands.** The page runs in the browser and invokes server
   ...
   the analysis handlers must be *actions*.
...
*validator*, handled by DL#179 alone):
...
settings-page *structure* this DL formalizes — `/wix-media/settings` (`devOnly: true`, component
...
would break the page UX. Settings pages need *actions*. `devOnly` is the missing axis — orthogonal to
action-vs-command: it says *"this handler exists only under the toolchain / dev server."*
...
A: No — `devOnly` is orthogonal to compiler use. Its meaning is *"admin/tooling surface, excluded from
production"*, not *"compiler-using."* `wix-media`'s settings actions are compiler-free yet still want
...
from production), it just doesn't *need* to for #179 leak reasons.
...
aiditor repos found **exactly one** plugin that is fully dev-only *and* has browser surfaces:
...
export type ActionManifestEntry =
    | string
    | { name: string; action?: string; devOnly?: boolean };
```
...
```yaml
routes:
  - { path: /design-system/settings, jayHtml: ./dist/pages/settings/page.jay-html, component: designSystemSettingsPage, devOnly: true }
actions:
