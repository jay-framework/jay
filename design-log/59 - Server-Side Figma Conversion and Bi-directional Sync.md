...
1.  **Vendor Client (e.g., Figma Plugin):** Acts as a "dumb" client. Its role is to serialize the editor's specific document structure into a JSON payload and send it to the server.
...
- **Client:** Serializes the design into a vendor-specific JSON (e.g., `FigmaDoc`).
...
3.  **Source Control:** Raw vendor files (`.figma.json`, `.wix.json`) are committed to Git, enabling version history for the _design_ source.
...
- **Decision:** The "Shared Schema" is shared _between the specific Vendor Client and its corresponding Server Adapter_.
  - Figma Client <-> Figma Adapter share `FigmaDoc`.
  - Wix Client <-> Wix Adapter share `WixDoc`.