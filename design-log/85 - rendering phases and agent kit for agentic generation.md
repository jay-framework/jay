...
2. **Contract discovery and materialization** (Design Log #80): `jay-stack contracts` materializes dynamic contracts; we output to **agent-kit** only (no separate `build/materialized-contracts/`). `contracts-index.yaml` and materialized contract files live under `agent-kit/`. Agents read these to understand data shapes and bindings.
...
Today an agent must:
- Know to run `jay-stack contracts` and read materialized contracts from **agent-kit**
...
| Contracts (materialized)         | `agent-kit/materialized-contracts/` + `contracts-index.yaml`; `jay-stack contracts` writes here (no build copy). Agents discover contracts from here. |
| Plugins index                    | Generated by `jay-stack contracts`; lists plugins (name, path, contracts). Single discovery point for which plugins/contracts exist.                   |
...
├── materialized-contracts/      # Output of jay-stack contracts (no build copy)
...
- **INSTRUCTIONS.md**: Summarize rendering phases (slow/fast/interactive), contract tags and phases, headless import + `jay:` instances, `key` and props, `forEach`/trackBy. List CLI: `jay-stack contracts` (writes to `agent-kit/materialized-contracts/` and generates plugins index), `jay-stack params <plugin>/<contract>`, `jay-stack action <plugin>/<action>`. Describe `src/pages` layout and that page dir can have `page.jay-html`, `page.jay-contract`, `page.conf.yaml` (Design Log #50).
- **materialized-contracts/**: Single place for materialized contracts and **plugins-index.yaml**; `jay-stack contracts` writes here (default or `--output agent-kit/materialized-contracts`). No copy in `build/`. Agents read contracts-index, plugins-index, and contract files from here.
...
2. Ensure contracts and plugins index exist: run `jay-stack contracts` (writes to `agent-kit/materialized-contracts/`: contracts-index.yaml, plugins-index.yaml, materialized contract files). Read contracts-index and plugins-index from there.
...
### Phase 1: Agent kit structure and instructions (no new tooling)
...
3. **Contract materialization**: `jay-stack contracts` writes to `agent-kit/materialized-contracts/` by default (no build copy) and generates **plugins-index.yaml** in the same folder. Design Log #80 implementation is extended so output directory defaults to or can be set to `agent-kit/materialized-contracts/`, and the command also emits a plugins index (plugin name, path, list of contracts).
...
1. **Default output**: `jay-stack contracts` defaults to `agent-kit/materialized-contracts/` (or `--output agent-kit/materialized-contracts`), writes contracts-index and **plugins-index**. No separate build copy.
...
3. Discover contracts and plugins: read contracts-index.yaml and plugins-index.yaml (run `jay-stack contracts` first).
...
1. An agent with access only to `agent-kit/` and the repo can, after running `jay-stack contracts` (output in `agent-kit/materialized-contracts/`, including plugins-index), produce valid jay-html for a given route using the right headless components and bindings.
...
- **Contract materialization** (#80): Materialized contracts and **plugins index** live in **agent-kit only**: `agent-kit/materialized-contracts/` (no build copy). `jay-stack contracts` writes contracts-index, **plugins-index**, and materialized contract files here.
...
- **Agent kit folder**: `agent-kit/` with INSTRUCTIONS.md, materialized-contracts (contracts-index, plugins-index, contract files), optional plugin refs, and **content/** as minimal spec so a coding agent can create jay-html consistently.
