...
function printNode(node, ident = '') {
    let nodeInfo = '';
    if (isInstanceNode(node)) {
        nodeInfo =
            `${node.mainComponent.type}, ${node.mainComponent.id}, ${getVariant(node)} wh:${node.width}:${node.height}, xy:${node.x}:${node.y}, cons:${JSON.stringify(node.constraints)}` +
            node.toString();
    }
    if (isComponentNode(node)) {
        nodeInfo =
            `${getVariant(node)} wh:${node.width}:${node.height}, xy:${node.x}:${node.y}, cons:${JSON.stringify(node.constraints)} ` +
            node.toString();
    }
    if (isGroupNode(node)) {
    ...
    if (isTextNode(node)) {
        nodeInfo =
            `wh:${node.width}:${node.height}, xy:${node.x}:${node.y}, ${JSON.stringify(node.constraints)} ` +
            node.toString();
    }
