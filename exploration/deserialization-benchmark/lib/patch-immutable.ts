        ...
        if (patchOperation.path.length - 1 === level) {
            if (patchOperation.op === REPLACE || patchOperation.op === ADD)
                ...
                if (Array.isArray(copy))
                    copy.splice(patchOperation.path[level] as number, 1)
                else
                    delete copy[patchOperation.path[level]];
            }
            else if (patchOperation.op === MOVE && Array.isArray(copy) && validateMove(patchOperation)) {
            ...
            prev[patchOperation.path[level]].push(patchOperation)
        return prev;
    }, {})
