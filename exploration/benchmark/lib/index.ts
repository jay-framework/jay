...
function createPatch(tableSize, patchSize) {
    ...
    for (let i = 0; i < patchSize; i++) {
        ...
        patch.push({op: REPLACE, path: [''+x, 'cell', ''+y, 'value'], value: Math.round(Math.random() * 100)})
    }
    return patch;
}
...
function benchmarkMutable() {
    ...
    console.log('end', end);
    console.log('average time to apply patches to mutable', (end - start) / BENCHMARK_CYCLES);
}
...
function benchmarkImmutable() {
    ...
    let end = performance.now();
    console.log('end', end);
    console.log('average time to apply patches to immutable', (end - start) / BENCHMARK_CYCLES);
}
