...
This workflow eliminates the manual translation of designs to code and ensures that the implemented components match the design specifications exactly.
...
- `names="Item"` - Name to use in the template
...
- `name` - Name of the exported component
- `key` - Attribute name under which the component's data is nested
...
export interface TodoElementRefs {
  ...
  items: ItemElementRefs[];
  filterAll: HTMLAnchorElement;
  filterActive: HTMLAnchorElement;
  filterCompleted: HTMLAnchorElement;
