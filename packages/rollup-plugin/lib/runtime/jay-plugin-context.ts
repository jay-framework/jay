...
} from "../../../compiler/lib/ts-file/building-blocks/compile-function-split-patterns.ts";
...
export class JayPluginContext {
    ...
    constructor(readonly jayOptions: JayRollupConfig = {}) {
        ...
        let compilerPatternsParseResult =
            compileFunctionSplitPatternsBlock(jayOptions.compilerPatternFiles);
        ...
        if (compilerPatternsParseResult.validations.length > 0)
            throw new Error('failed to parse or validate compilerPatternFiles. \n' + compilerPatternsParseResult.validations.join('\n'))
