...
export function jayEventHandlerToReact(
    viewState: any,
    coordinate,
    eventType: string,
    ...
    return handler
        ? React.useCallback(
              ...
              [viewState, ...coordinate, eventType],
          )
        : undefined;
}

export function createElementFromJay(
    elType: string,
    viewState: any,
    ...
    const allProps = {
        ...props,
        ...Object.fromEntries(
            Object.entries(events || {}).map(([eventType, handler]) => {
                const reactEventType = eventJayToReactMap[eventType];
                const reactHandler = jayEventHandlerToReact(
                    viewState,
                    coordinate,
                    eventType,
                    handler,
