...
describe('RouteScanner', () => {
    ...
    it('should not have params for routes without jay-params tag', async () => {
        ...
        const productsRoute = routes.find((r) => r.rawRoute === '/products');
        expect(productsRoute?.inferredParams).toBeUndefined();
    });
});
...
describe('jay-params parsing', () => {
    ...
    it('should handle jay-html without head tag', async () => {
        const routes = await scanRoutes('./test/fixtures/jay-params', options);
