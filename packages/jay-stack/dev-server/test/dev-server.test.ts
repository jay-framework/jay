...
describe('DevServerService', () => {
    ...
    it('refreshRoutes registers pages created after startup', async () => {
        const fs = await import('node:fs/promises');
        const os = await import('node:os');
        const tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'jay-dev-routes-'));
        ...
        expect(devServer.service.listRoutes().map((r) => r.path)).toEqual(['/home']);
        ...
        expect(routes.map((r) => r.path).sort()).toEqual(['/home', '/pricing']);
