...
describe('Multiple Servers and Clients', () => {
  ...
  it('should handle concurrent connections to multiple servers', async () => {
    ...
    initServer1.onPublish(async () => ({ status: [{ success: true, filePath: '/init1.jay-html' }] }));
    initServer2.onPublish(async () => ({ status: [{ success: true, filePath: '/init2.jay-html' }] }));
    configuredServer1.onPublish(async () => ({ status: [{ success: true, filePath: '/config1.jay-html' }] }));
    configuredServer2.onPublish(async () => ({ status: [{ success: true, filePath: '/config2.jay-html' }] }));
