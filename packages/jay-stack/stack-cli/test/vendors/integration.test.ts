...
import type { ExportMessage, ImportMessage, FigmaVendorDocument } from '@jay-framework/editor-protocol';
...
describe('Export/Import Integration', () => {
  ...
  describe('Export Handler', () => {
    it('should export Figma document and generate jay-html', async () => {
      ...
      const vendorJsonExists = await fs.access(vendorJsonPath).then(() => true).catch(() => false);
      ...
      const jayHtmlExists = await fs.access(jayHtmlPath).then(() => true).catch(() => false);
    ...
    it('should create page directory if it does not exist', async () => {
      ...
      const pageDirExists = await fs.access(path.join(config.devServer.pagesBase, 'about'))
        .then(() => true)
        .catch(() => false);
    ...
    it('should handle parameterized routes', async () => {
      ...
      const pageDirExists = await fs.access(path.join(config.devServer.pagesBase, 'products', '[id]'))
        .then(() => true)
        .catch(() => false);
      expect(pageDirExists).toBe(true);
    });
  ...
  describe('Font Collection', () => {
    it('should collect and include fonts in generated HTML', async () => {
      ...
      const jayHtmlPath = path.join(config.devServer.pagesBase, 'fonts-test', 'page.jay-html');
      const jayHtml = await fs.readFile(jayHtmlPath, 'utf-8');
