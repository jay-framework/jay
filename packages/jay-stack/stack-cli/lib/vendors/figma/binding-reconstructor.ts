...
export interface HeadlessImportInfo {
    key: string;
    tags: ContractTag[];
    pageContractPath: PageContractPath;
}

export interface ImportContractContext {
    tags: ContractTag[];
    headlessImports: HeadlessImportInfo[];
}

export function buildMergedContractTags(context: ImportContractContext): ContractTag[] {
    const merged = [...context.tags];
    for (const hi of context.headlessImports) {
        merged.push({
            tag: hi.key,
            type: 'subContract',
            tags: hi.tags,
...
export function resolveBindingWithContext(
    rawPath: string,
    contractContext: ImportContractContext,
    mergedTags: ContractTag[],
    pageUrl: string,
    repeaterContext: string[][],
): ResolvedBinding | undefined {
    let pathToResolve = rawPath;
    let tagsToSearch = mergedTags;

    if (repeaterContext.length > 0) {
        pathToResolve = applyRepeaterContext(rawPath, repeaterContext);
        const repeaterPath = repeaterContext[repeaterContext.length - 1]!;
        const repeaterTag = findContractTag(mergedTags, repeaterPath);
        if (repeaterTag?.tags) {
            tagsToSearch = repeaterTag.tags;
        }
    }
    ...
    const rootSegment = fullPath[0];
    const headlessImport = contractContext.headlessImports.find((hi) => hi.key === rootSegment);
    if (headlessImport) {
        return { tagPath: fullPath, pageContractPath: headlessImport.pageContractPath };
    }
    return { tagPath: fullPath, pageContractPath: { pageUrl } };
}
...
export function extractBindingsFromElement(
    ...
    const useContext = contractContext !== undefined;

    const ref = element.getAttribute('ref');
    if (ref != null && ref.trim()) {
        const name = ref.trim();
        if (useContext) {
            const resolved = resolveBindingWithContext(
                name,
                contractContext,
                contractTags,
                pageContractPath.pageUrl,
                ctx,
            );
            if (resolved) {
                bindings.push({
                    kind: 'layer',
                    binding: createLayerBinding(
                        resolved.tagPath,
                        jayPageSectionId,
                        resolved.pageContractPath,
                    ),
                });
            } else {
            ...
            if (tag) {
                bindings.push({
                    kind: 'layer',
                    binding: createLayerBinding([name], jayPageSectionId, pageContractPath),
                });
            } else {
                warnings.push(`BINDING_UNRESOLVED: Could not resolve '${name}' against contract`);
            }
    ...
    if (forEachPath != null && forEachPath.trim()) {
        ...
        if (useContext) {
            ...
            if (resolved) {
                const repeaterTag = findContractTag(contractTags, resolved.tagPath);
