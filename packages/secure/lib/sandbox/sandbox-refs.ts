...
export function elemRef(refName: string): SecureElementRef<any, any> {
    ...
    return refManager.add(
        refName,
        new StaticRefImplementation(refName, endpoint, viewState, coordinate),
    );
}

export function elemCollectionRef<ViewState, ElementType extends HTMLElement>(
    refName: string,
): () => SecureElementRef<ViewState, any> {
    ...
    return () => {
        ...
        let coordinate = [...dataIds, refName];
        let ref = new StaticRefImplementation<any, ElementType>(
            refName,
            endpoint,
            viewState,
            coordinate,
...
export function compRef(refName: string): PrivateRef<any, any> {
    ...
    return refManager.add(
        refName,
        new ComponentRefImpl(viewState, coordinate, refManager.eventWrapper),
    );
}
...
export class StaticRefImplementation<ViewState, ElementType extends HTMLElement>
    implements
...
export function newSecureHTMLElementPublicApiProxy<
    ...
    Target extends
        | StaticRefImplementation<ViewState, ElementType>
        | DynamicRefImplementation<ViewState, ElementType>,
...
export class DynamicRefImplementation<ViewState, ElementType extends HTMLElement>
    implements
        ...
        ManagedRef<HTMLElementCollectionProxy<ViewState, ElementType>>
{
    ...
    addEventListener<E extends Event>(
        type: string,
        listener: JayEventHandler<E, any, any> | null,
        options?: boolean | AddEventListenerOptions,
        nativeId?: string,
    ): void {
        this.ep.post(addEventListenerMessage(this.ref, type, nativeId));
                    ...
                    case JayPortMessageType.eventInvocation: {
                        reactive.batchReactions(() => {
                            (
                                ...
                                ) as StaticRefImplementation<ViewState, any>
                            ).invoke(
                                inMessage.eventType,
