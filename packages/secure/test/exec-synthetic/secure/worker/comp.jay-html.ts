...
export function render(options?: RenderElementOptions): CompElementPreRender {
    const [refManager, [result, buttonExecGlobal, buttonExecElement, itemButtonExecElement  ]] =
        SecureReferencesManager.forElement(['result', 'buttonExecGlobal', 'buttonExecElement'], ['itemButtonExecElement'], [], []);
    const render = (viewState: CompViewState) => elementBridge(viewState, refManager, () => {
        return [
            e(result()),
            e(buttonExecGlobal()),
            e(buttonExecElement()),
