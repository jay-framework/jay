...
export function render(viewState: ChildViewState, options?: RenderElementOptions): ChildElement {
  return ConstructContext.withRootContext(viewState, () =>
      e('div', {}, [
          ...
          e('button', {id: da(vs => `event-to-parent-button-${vs.id}`)}, ['event to parent'], er('eventToParent')),
          e('button', {id: da(vs => `event-to-parent-to-child-prop-button-${vs.id}`)}, ['event to parent, parent update child prop'], er('eventToParentToChildProp')),
          e('button', {id: da(vs => `event-to-parent-to-child-api-button-${vs.id}`)}, ['event to parent, parent calls child api'], er('eventToParentToChildApi'))
      ]), options);
}