...
describe('sandbox-refs', () => {
    describe('static refs', () => {
        ...
        it('should remove event listener using removeEventListener', () => {
            ...
            (bridgeElement.refs.one as HTMLElementProxy<any, any>).removeEventListener(
                'click',
                callback,
            );
        ...
        it('should run $exec --> JPMNativeExec', () => {
            ...
            (bridgeElement.refs.one as HTMLElementProxy<ViewState, HTMLDivElement>).exec$(
                func$('3'),
            );
    ...
    describe('dynamic forEach refs - one level', () => {
        ...
        it('should trigger events on JPMDomEvent --> callback', () => {
            ...
            expect(callback.mock.calls[0][0]).toEqual({
                coordinate: ['B', 'one'],
                event: undefined,
                viewState: baseViewState.items[1],
            });
        });
        ...
        it('should support viewState updates - additional item', () => {
            ...
            expect(callback.mock.calls[0][0]).toEqual({
                coordinate: ['D', 'one'],
                event: undefined,
                viewState: addItemViewState.items[3],
            });
        });
        ...
        describe('map', () => {
            it('should run map handler on all items - passing view state and coordinate', () => {
                ...
                (bridgeElement.refs.one as HTMLElementCollectionProxy<Item, HTMLDivElement>).map(
                    (element, viewState, coordinate) => {
    ...
    describe('dynamic foreach refs - multi-level', () => {
        ...
        it('should trigger events on JPMDomEvent --> callback', () => {
            ...
            expect(callback.mock.calls[0][0]).toEqual({
                coordinate: ['B', '3', 'two'],
                event: undefined,
                viewState: vs.items[1].subItems[0],
            });
        });
        ...
        it('should support view state updates', () => {
            ...
            expect(callback.mock.calls[0][0]).toEqual({
                coordinate: ['A', '2.5', 'two'],
                event: undefined,
                viewState: vs2.items[0].subItems[2],
            });
            expect(callback.mock.calls[1][0]).toEqual({
                coordinate: ['C', '5', 'two'],
                event: undefined,
                viewState: vs2.items[2].subItems[0],
            });
            expect(callback.mock.calls[2][0]).toEqual({
                coordinate: ['E', '9', 'two'],
                event: undefined,
                viewState: vs2.items[3].subItems[0],
            });
        });
    });
    ...
    describe('dynamic foreach + condition', () => {
        ...
        it('should trigger events with view state for mounted elements, and with undefined for unmounted elements (parent condition === false)', () => {
            ...
            expect(callback.mock.calls[0][0]).toEqual({
                coordinate: ['A', 'two'],
                event: undefined,
                viewState: vs.items[0],
            });
            expect(callback.mock.calls[1][0]).toEqual({
                coordinate: ['B', 'two'],
                event: undefined,
                viewState: vs.items[1],
            });
        });
        ...
        it('after update, should trigger events with view state for mounted elements, and with undefined for unmounted elements (parent condition === false)', () => {
            ...
            expect(callback.mock.calls[0][0]).toEqual({
                coordinate: ['A', 'two'],
                event: undefined,
                viewState: vs2.items[0],
            });
            expect(callback.mock.calls[1][0]).toEqual({
                coordinate: ['E', 'two'],
                event: undefined,
                viewState: vs2.items[3],
            });
        });
    });
    ...
    describe('static component', () => {
        ...
        it('should create the component with the provided props', () => {
            ...
            expect(childCompRef.getItemSummary()).toBe(
                'item some data - Done: false - mounted: true',
            );
        });
        ...
        it('should update the component with the new props', () => {
            ...
            expect(childCompRef.getItemSummary()).toBe(
                'item some new data - Done: false - mounted: true',
            );
        });
        ...
        it('should support component APIs', () => {
            ...
            expect(childCompRef.getItemSummary()).toBe(
                'item some data - Done: true - mounted: true',
            );
        });
    });
    ...
    describe('dynamic conditional component', () => {
        ...
        it('should re-mount the component if under true condition', () => {
            ...
            expect(instance.getItemSummary()).toBe(
                'item some new data - Done: false - mounted: true',
            );
        });
    });
    ...
    describe('dynamic forEach components', () => {
        ...
        it('should create the components with the provided props', () => {
            ...
            expect(componentInstance(A.dataId).getItemSummary()).toBe(
                'item some data - Done: false - mounted: true',
            );
            expect(componentInstance(B.dataId).getItemSummary()).toBe(
                'item B data - Done: false - mounted: true',
            );
            expect(componentInstance(C.dataId).getItemSummary()).toBe(
                'item C data - Done: false - mounted: true',
            );
        });
        ...
        it('should update the components with the new instances and props, unmounting removed components', () => {
            ...
            expect(componentInstance(B.dataId).getItemSummary()).toBe(
                'item B data - Done: false - mounted: false',
            );
            expect(componentInstance(C.dataId).getItemSummary()).toBe(
                'item C data - Done: false - mounted: true',
            );
            expect(componentInstance(D.dataId).getItemSummary()).toBe(
                'item D data - Done: false - mounted: true',
            );
        });
        ...
        it('should support map components', () => {
            ...
            expect(mapResult)
                .toContain(`comp.getItemSummary(): item some data - Done: false - mounted: true
viewState: ${JSON.stringify(A)}
coordinate: [a,comp1]`);
            expect(mapResult)
                .toContain(`comp.getItemSummary(): item B data - Done: false - mounted: true
viewState: ${JSON.stringify(B)}
coordinate: [b,comp1]`);
            expect(mapResult)
                .toContain(`comp.getItemSummary(): item C data - Done: false - mounted: true
