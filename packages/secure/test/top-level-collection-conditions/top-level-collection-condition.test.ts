...
describe('top level collections and conditions', () => {
    async function mkElement(viewState: AppViewState) {
        ...
        let add = (id) =>
            appElement.dom.querySelector(`[data-id="${id}-add"]`) as HTMLButtonElement;
        let sub = (id) =>
            appElement.dom.querySelector(`[data-id="${id}-sub"]`) as HTMLButtonElement;
        let count = (id) =>
            appElement.dom.querySelector(`[data-id="${id}-count"]`) as HTMLSpanElement;
    ...
    it('should render 3 counter components', async () => {
        ...
        expect(count(COUNTER_A_ID).textContent).toBe(
            '' + initialCountById(viewState, COUNTER_A_ID),
        );
        expect(title(COUNTER_B_ID).textContent).toBe(collCounterTitle(COUNTER_B_ID));
        expect(count(COUNTER_B_ID).textContent).toBe(
            '' + initialCountById(viewState, COUNTER_B_ID),
        );
    });
    ...
    it('the counter components should work', async () => {
        ...
        expect(title(COUNTER_A_ID).textContent).toBe(collCounterTitle(COUNTER_A_ID));
        expect(count(COUNTER_A_ID).textContent).toBe(
            '' + (initialCountById(viewState, COUNTER_A_ID) + 1),
        );
        expect(title(COUNTER_B_ID).textContent).toBe(collCounterTitle(COUNTER_B_ID));
        expect(count(COUNTER_B_ID).textContent).toBe(
            '' + (initialCountById(viewState, COUNTER_B_ID) - 2),
        );
    });
    ...
    it('should not render the cond counter if condition === false', async () => {
        ...
        expect(title(COUNTER_A_ID).textContent).toBe(collCounterTitle(COUNTER_A_ID));
        expect(count(COUNTER_A_ID).textContent).toBe(
            '' + initialCountById(viewState2, COUNTER_A_ID),
        );
        expect(title(COUNTER_B_ID).textContent).toBe(collCounterTitle(COUNTER_B_ID));
        expect(count(COUNTER_B_ID).textContent).toBe(
            '' + initialCountById(viewState2, COUNTER_B_ID),
        );
    });
    ...
    it('should update the collection counters', async () => {
        ...
        expect(title(COUNTER_A_ID).textContent).toBe(collCounterTitle(COUNTER_A_ID));
        expect(count(COUNTER_A_ID).textContent).toBe(
            '' + initialCountById(viewState3, COUNTER_A_ID),
        );
        expect(title(COUNTER_C_ID).textContent).toBe(collCounterTitle(COUNTER_C_ID));
        expect(count(COUNTER_C_ID).textContent).toBe(
            '' + initialCountById(viewState3, COUNTER_C_ID),
        );
    });
    ...
    it('supports root component APIs for collection component', async () => {
        ...
        expect(
            await appElement.refs.comp2.find((_) => _.id === COUNTER_A_ID).counterDescription(),
    ...
    it('supports root component events for conditional component', async () => {
        ...
        expect(fn).toBeCalledWith({
            event: { value: viewState.initialCount + 1 },
            viewState,
            coordinate: COUNTER_COND_COORDINATE,
        });
    });
    ...
    it('supports root component events for collection component', async () => {
        ...
        expect(fn).toBeCalledTimes(1);
        expect(fn).toBeCalledWith({
            event: { value: COUNTER_A.initialCount + 1 },
            viewState: COUNTER_A,
            coordinate: COUNTER_A_COORDINATE,
        });
    });
});
