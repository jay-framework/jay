...
class JayMockLogger implements JayPortLogger {
    ...
    logInvoke(compId: number, message: JPMMessage, endpointFound: boolean): void {
        if (this.verbose)
            console.log(
                `${this.name}.invoke - compId: ${compId} ${
                    endpointFound ? '' : '[COMP NOT FOUND] '
                }type: ${describeMessageType(message.type)} message: ${JSON.stringify(message)}`,
            );
    }
    ...
    logPost(compId: number, message: JPMMessage): void {
        if (this.verbose)
            console.log(
                `${this.name}.post - compId: ${compId} type: ${describeMessageType(
                    message.type,
...
class JayMockChannel2 implements MockJayChannel {
    ...
    mainPostingMessages(
        ...
        process.nextTick(() => {
            ...
            messages.forEach(
                ([compId, message]) =>
                    (this.messageLog.find((item) => item[0] === message)[1] = 'invoked'),
            );
            this.messageCountCallback(-1);
        });
    }
    ...
    messageCountCallback = (diff) => {
        if (this.pendingMessages === 0) [this.dirty, this.dirtyResolve] = mkResolvablePromise();
        this.pendingMessages += diff;
        if (this.pendingMessages === 0) this.dirtyResolve();
    };
