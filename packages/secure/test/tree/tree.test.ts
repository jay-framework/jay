...
describe('events synthetic tests', () => {
    async function mkElement(viewState: Node) {
        ...
        let getHeadArrowById = (id) =>
            appElement.dom.querySelector(`[data-ref="head=${id}"] .tree-arrow`) as HTMLSpanElement;
        let getHeadNameById = (id) =>
            appElement.dom.querySelector(`[data-ref="head=${id}"] .name`) as HTMLSpanElement;
        let getListById = (id) =>
            appElement.dom.querySelector(`[data-ref="list=${id}"]`) as HTMLUListElement;
        ...
        let expectTreeNode = (node: Node, arrow: string, hasChildren: boolean) => {
            ...
            if (hasChildren) expect(getListById(node.id)).toBeDefined();
            else expect(getListById(node.id)).toBeNull();
        };
    ...
    describe('two level tree', () => {
        ...
        it('reorder tree children by moving them, not recreating / overriding, and not including empty patches', async () => {
            ...
            VERBOSE && console.log(appElement.dom.outerHTML);
            expect(channel.messageLog).toContainEqual([
                renderMessage([
    ...
    describe('tree level tree', () => {
        ...
        it('expand the whole tree and update tree', async () => {
            let { channel, expectTreeNode, expectNoTreeNode, appElement, getHeadById } =
                await mkElement(Tree3_1);
