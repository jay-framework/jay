...
import {BasicElement, BasicViewState} from "../worker/basic.jay.html";
...
class MainRoot<AppProps> {
    ...
    start(props: AppProps) {
        ...
        this.element = render(viewState as BasicViewState)
    }

    update(props: AppProps) {
        let viewState = this.mainPort.update(props);
        this.element.update(viewState as BasicViewState)
    }
}
