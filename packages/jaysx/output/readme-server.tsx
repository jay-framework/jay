...
const CartComp = Jay(async ()=>{
    ...
    return <div>
        <div>{Jay.map(state ,item=><CartItemComp key={`CartItemComp0:${item.id}`} workerRendered />)}</div>
    </div>
})
