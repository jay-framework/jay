...
describe('list-compare', () => {
    ...
    it('should optimize move instruction a moved item forward', () => {
        ...
        expect(instructions.length).toBe(1);
        expect(instructions).toEqual([{ action: ITEM_MOVED, pos: 4, fromPos: 1 }]);
        expect(mutatedList).toEqual(newList.toArray());
        expect(toUpdateList.toArray()).toEqual(newList.toArray());
    });

    it('should optimize multiple move instruction sequences', () => {
        let oldList = new List<Item, void>(
            [itemA, itemB, itemC, itemD, itemE, item3, item18, item21, item36, item39],
            'id',
        );
        let toUpdateList = oldList.clone();
        let newList = new List<Item, void>(
            [itemA, itemC, itemD, itemE, itemB, item3, item21, item36, item39, item18],
            'id',
        );
    ...
    it('should return instructions for move then remove', () => {
        let oldList = new List<Item, void>(
            [item18, item21, item36, item39, item42, item3, item48],
            'id',
        );
        let toUpdateList = oldList.clone();
    ...
    it('optimize sequence in the middle of instructions array', () => {
        ...
        expect(instructions).toEqual([
            {
                action: 'IR',
                item: { name: 'item 33', completed: true, cost: 33, id: 'a33' },
                pos: 1,
            },
            { action: 'IM', pos: 5, fromPos: 2 },
            {
                action: 'IA',
                item: { name: 'item 210', completed: false, cost: 210, id: 'a210' },
                pos: 5,
            },
        ]);
        expect(mutatedList).toEqual(newList.toArray());
