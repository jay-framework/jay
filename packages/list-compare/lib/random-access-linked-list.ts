...
export class RandomAccessLinkedList<T, S> implements UntypedRandomAccessLinkedList {
    ...
    constructor(arr: Array<T>, matchBy: string) {
        ...
        this._first = arr.reduceRight(
            (
                nextItem: LinkedListItem<T, S> | typeof EoF,
                obj: T,
            ): LinkedListItem<T, S> | typeof EoF => {
                let item: LinkedListItem<T, S> = {
                    id: obj[matchBy],
                    value: obj,
                    next: nextItem,
                    prev: BoF,
                };
                if (nextItem !== EoF) (nextItem as LinkedListItem<T, S>).prev = item;
                if (this._last === BoF) this._last = item;
