...
describe('diff', () => {
    describe('new object', () => {
        it('should return ADD patch for a new object', () => {
            ...
            expect(patch[0]).toEqual([{ op: ADD, path: [], value: { a: 1, b: 2 } }]);
        });
    });
    ...
    describe('atomic values', () => {
        ...
        it('should return replace patch different values', () => {
            let patch = diff(1, 2);
            expect(patch[0]).toEqual([{ op: REPLACE, path: [], value: 1 }]);
        });
    });

    describe('objects values', () => {
        it('should return empty for equal objects', () => {
            let patch = diff({ a: 1, b: 2, c: { d: 4, e: 5 } }, { a: 1, b: 2, c: { d: 4, e: 5 } });
            expect(patch[0]).toEqual([]);
        });

        it('should return replace for top level property replacement', () => {
            let patch = diff({ a: 1, b: 4, c: { d: 4, e: 5 } }, { a: 1, b: 2, c: { d: 4, e: 5 } });
            expect(patch[0]).toEqual([{ op: REPLACE, path: ['b'], value: 4 }]);
        });

        it('should return replace for deep property replacement', () => {
            let patch = diff({ a: 1, b: 2, c: { d: 7, e: 5 } }, { a: 1, b: 2, c: { d: 4, e: 5 } });
            expect(patch[0]).toEqual([{ op: REPLACE, path: ['c', 'd'], value: 7 }]);
        });
        ...
        it('should return add for new keys', () => {
            ...
            expect(patch[0]).toEqual([{ op: ADD, path: ['f'], value: 6 }]);
        });
        ...
        it('should return add for new deep keys', () => {
            ...
            expect(patch[0]).toEqual([{ op: ADD, path: ['c', 'f'], value: 6 }]);
        });
        ...
        it('should return remove for removed keys', () => {
            ...
            expect(patch[0]).toEqual([{ op: REMOVE, path: ['f'] }]);
        });
        ...
        it('should return remove for removed deep keys', () => {
            ...
            expect(patch[0]).toEqual([{ op: REMOVE, path: ['c', 'f'] }]);
        });

        it('should not remove property values that evaluate as false', () => {
            let patch = diff({ a: 1, b: 0 }, { a: 1, b: 0 });
            expect(patch[0]).toEqual([]);
        });

        it('should return replace object with primitive', () => {
            let patch = diff({ a: 1, b: 2, c: 4 }, { a: 1, b: 2, c: { d: 4, e: 5, f: 6 } });
            expect(patch[0]).toEqual([{ op: REPLACE, path: ['c'], value: 4 }]);
        });

        it('should return replace primitive with object', () => {
            let patch = diff({ a: 1, b: 2, c: { d: 4, e: 5, f: 6 } }, { a: 1, b: 2, c: 4 });
            expect(patch[0]).toEqual([{ op: REPLACE, path: ['c'], value: { d: 4, e: 5, f: 6 } }]);
        });
        ...
        it("should return replace an object if more then 50% of it's properties has been modified", () => {
            ...
            expect(patch[0]).toEqual([{ op: REPLACE, path: ['c'], value: { d: 7, e: 8, f: 9 } }]);
        });
    ...
    describe('primitive array values', () => {
        it('should return empty for the same array', () => {
            let patch = diff([1, 2, 3], [1, 2, 3], []);
            expect(patch[0]).toEqual([]);
        });

        it('should return patch for added item', () => {
            let patch = diff([1, 2, 3, 4], [1, 2, 3], []);
            expect(patch[0]).toEqual([{ op: ADD, path: ['3'], value: 4 }]);
        });

        it('should return patch for removed item', () => {
            let patch = diff([1, 2, 3], [1, 2, 3, 4], []);
            expect(patch[0]).toEqual([{ op: REMOVE, path: ['3'] }]);
        });

        it('should return patch for changed item', () => {
            let patch = diff([1, 2, 3], [1, 4, 3], []);
            expect(patch[0]).toEqual([{ op: REPLACE, path: ['1'], value: 2 }]);
        });

        it('should replace all the array if there is a large change', () => {
            let patch = diff([1, 2, 3], [4, 5, 6], []);
            expect(patch[0]).toEqual([{ op: REPLACE, path: [], value: [1, 2, 3] }]);
        });

        it('should return patch for changes in a nested array', () => {
            let patch = diff({ a: [1, 2, 3, 5] }, { a: [1, 4, 3] }, []);
            expect(patch[0]).toEqual([
    ...
    describe('object array values', () => {
        describe('with an array context', () => {
            const TOP_LEVEL_ARRAY_CONTEXT: ArrayContexts = [[[], { matchBy: 'id' }]];
            ...
            it('should return add patch for an added item', () => {
                ...
                expect(patch[0]).toEqual([{ op: ADD, path: [3], value: { id: 4, c: '4' } }]);
            });
            ...
            it('should return move patch for a moved item', () => {
                ...
                expect(patch[0]).toEqual([{ op: MOVE, path: [1], from: [2] }]);
            });
            ...
            it('should return a patch for object property update', () => {
                ...
                expect(patch[0]).toEqual([{ op: REPLACE, path: ['1', 'c'], value: '4' }]);
            });
