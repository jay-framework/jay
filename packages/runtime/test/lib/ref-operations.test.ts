...
describe('ReferencesManager operations', () => {
    ...
    describe('single referenced element', () => {
        ...
        function mkElement() {
            let jayElement1;
            let jayRootElement = ConstructContext.withRootContext<string, RootElementRefs>(DATA_CONTEXT, () =>
                e('div', {}, [
                    jayElement1 = e('div', {}, [SOME_VALUE], elemRef(refName1))]));
    ...
    describe('dynamic list of referenced elements', () => {
        ...
        it("find should find the first element proxy meeting a criteria based on view state", () => {
            let {jayRootElement, jayElements, mockCallback} = mkJayElement();
            let element2 =
                jayRootElement.refs.refName1.find(vs => vs === VIEW_STATE.items[1])
        ...
        it("find should find the first element proxy meeting a criteria based on coordinate", () => {
            let {jayRootElement, jayElements, mockCallback} = mkJayElement();
            let element2 =
                jayRootElement.refs.refName1.find(
                    (vs, coordinate) => sameCoordinate(coordinate, COORDINATE_12))
            ...
            jayRootElement = ConstructContext.withRootContext(DATA_CONTEXT, () =>
              e('div', {}, [
                  childComp((props: ItemProps) => jayComponent = Item(props),
                    vs => ({text: 'hello', dataId: 'AAA'}), compRef(refName1))])) as JayElement<RootElementViewState, RootElementRefs>;
    ...
    describe('dynamic list of referenced component', () => {
        ...
        interface RootElementRefs {
            refName1: ComponentCollectionProxy<typeof viewState.items[0], ItemRef<RootElementViewState>>
        }
            ...
            jayRootElement = ConstructContext.withRootContext(viewState, () => {
                ...
                return de('div', {}, [
                    forEach((vs: typeof viewState) => vs.items,
                        (item) =>
                            childComp((props) => Item(props as ItemProps), (vs: RootElementViewStateItem) => vs.props, ref1()),
                        'id')
                ])
        ...
        it('map should allow using component APIs and return array of callback return values', () => {
            let summaries = jayRootElement.refs.refName1.map((comp, vs, coordinate) => comp.getItemSummary())
        ...
        it('map should provide viewState and coordinate', () => {
            ...
            expect(coordinates).toEqual([["1","refName1"], ["2","refName1"], ["3","refName1"]]);
        })

        it('should find elements based on viewState', () => {
            let foundComp = jayRootElement.refs.refName1.find((vs) => vs.id === viewState.items[1].id)
