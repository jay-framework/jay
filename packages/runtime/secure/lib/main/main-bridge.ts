                    ...
                    try {
                        let result = ref.find
                            ? await (ref as HTMLElementCollectionProxy<any, any>)
                                  ...
                                  .exec$((elem, vs) =>
                                      (
                                          funcRepository[nativeId] as JayNativeFunction<
                                              any,
                                              any,
                                              any
                                          >
                            ...
                            : await ref.exec$((elem, vs) =>
                                  (funcRepository[nativeId] as JayNativeFunction<any, any, any>)(
                                      elem,
                                      vs,
                                  ),
                              );
                        ...
                        endpoint.post(
                            nativeExecResult(correlationId, undefined, err.message, refName),
                        );
                    }
                });
            }
...
function defineCompPublicAPI(
    comp: MainComponentBridge,
    endpoint: IJayEndpoint,
    options: CompBridgeOptions,
) {
    if (options?.events)
        comp['addEventListener'] = (eventType: string, handler: Function) =>
            comp.registerEvent(eventType, handler);
