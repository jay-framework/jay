...
import {
    readGeneratedElementBridgeFile, readGeneratedElementDefinitionFile,
    readGeneratedNamedFile,
    readNamedSourceJayFile,
    readSourceJayFile, readTestFile
} from './test-fs-utils';
    ...
    describe('sandboxed counter', () => {
        ...
        describe('sandbox target', () => {
            it('generates sandbox root', async () => {
                const jayFile = await readNamedSourceJayFile('sandboxed/sandboxed-counter/source', 'app');
                ...
                expect(sandboxRootFile.val).toEqual(
                    await readGeneratedNamedFile('sandboxed/sandboxed-counter/generated/sandbox', 'sandbox-root'),
                );
            })

            it('generates counter element', async () => {
                const jayFile = await readNamedSourceJayFile('sandboxed/sandboxed-counter/source', 'counter');
                let runtimeFile = generateElementBridgeFile(jayFile, 'counter.jay.html', './sandboxed/sandboxed-counter-source');
                expect(runtimeFile.validations).toEqual([]);
                expect(runtimeFile.val).toEqual(
                    await readGeneratedNamedFile('sandboxed/sandboxed-counter/generated/sandbox', 'counter.jay.html'),
                );
            });
        })
        ...
        describe('source (dev) target', () => {
            it('generates element definition file', async () => {
                ...
                expect(runtimeFile.validations).toEqual([]);
                expect(runtimeFile.val).toEqual(
                    await readGeneratedNamedFile('sandboxed/sandboxed-counter/source', 'counter.jay.html.d'),
                );
            }, 10000);
        })
        ...
        describe('main target', () => {
            it('generates app element file', async () => {
                ...
                expect(runtimeFile.validations).toEqual([]);
                expect(runtimeFile.val).toEqual(
                    await readGeneratedNamedFile('sandboxed/sandboxed-counter/generated/main', 'app.jay.html')
                );
            })
            ...
            it('generates counter element file', async () => {
                ...
                expect(runtimeFile.validations).toEqual([]);
                expect(runtimeFile.val).toEqual(
                    await readGeneratedNamedFile('sandboxed/sandboxed-counter/generated/main', 'counter.jay.html')
                );
            })

            it('generates counter refs file', async () => {
                let refsFile = generateComponentRefsDefinitionFile('./test/fixtures/sandboxed/sandboxed-counter/source/counter');
                expect(refsFile.validations).toEqual([]);
                expect(refsFile.val).toEqual(
                    await readTestFile('./sandboxed/sandboxed-counter/generated/main', 'counter-refs.d.ts'),
                );
            });

            it('generates counter bridge', async () => {
                const sourceFile = await readTsSourceFile('sandboxed/sandboxed-counter/source', 'counter.ts');
                ...
                expect(outputCode).toEqual(
                    await readExpectedTsFile('sandboxed/sandboxed-counter/generated/main', 'counter.ts'),
                );
            })
