    ...
    describe('analyze read patterns', () => {
        describe('test the getExpressionStatus and getStatementStatus APIs', () => {
            it('should analyze inline (arrow) event handler with access to event.target.value', async () => {
                ...
                const eventHandlerBody = (
                    (sourceFile.statements[1] as ExpressionStatement).expression as ArrowFunction
                ).body as CallExpression;
                const eventTargetValueExpression = eventHandlerBody.arguments[0];

                expect(analyzedFile.getExpressionStatus(eventTargetValueExpression)).toEqual({
                    expression: eventTargetValueExpression,
                    patterns: [patterns[0]],
                    testId: 0,
                });

                expect(analyzedFile.getStatementStatus(sourceFile.statements[1])).toEqual({
            ...
            it('should analyze block event handler with access to event.target.value', async () => {
                ...
                const eventTargetValueExpression = (
                    (eventHandlerBody.statements[0] as ExpressionStatement)
                ...
                expect(analyzedFile.getStatementStatus(eventHandlerBody.statements[0])).toEqual({
                    targetEnv: JayTargetEnv.sandbox,
                    matchedPatterns: [
                        {
                            expression: eventTargetValueExpression,
        ...
        it('basic read pattern', async () => {
            ...
            expect(await printAnalyzedExpressions(analyzedFile)).toEqual(
                new Set(['0: (event.target as HTMLInputElement).value; matches inputValuePattern']),
            );
        });
