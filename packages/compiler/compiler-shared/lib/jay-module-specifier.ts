 ...
 */
const JAY_QUERY_PATTERNS: Array<{ pattern: string; buildEnv?: JayBuildEnvironment; runtimeMode?: RuntimeMode }> = [
    // Build environments
    { pattern: `${JAY_QUERY_PREFIX}${JayBuildEnvironment.Client}`, buildEnv: JayBuildEnvironment.Client },
    { pattern: `${JAY_QUERY_PREFIX}${JayBuildEnvironment.Server}`, buildEnv: JayBuildEnvironment.Server },
    // Runtime modes (with .ts suffix)
    { pattern: `${JAY_QUERY_PREFIX}${RuntimeMode.MainSandbox}${TS_EXTENSION}`, runtimeMode: RuntimeMode.MainSandbox },
    { pattern: `${JAY_QUERY_PREFIX}${RuntimeMode.WorkerTrusted}${TS_EXTENSION}`, runtimeMode: RuntimeMode.WorkerTrusted },
    { pattern: `${JAY_QUERY_PREFIX}${RuntimeMode.WorkerSandbox}${TS_EXTENSION}`, runtimeMode: RuntimeMode.WorkerSandbox },
    // Runtime modes (without .ts suffix)
    { pattern: `${JAY_QUERY_PREFIX}${RuntimeMode.MainSandbox}`, runtimeMode: RuntimeMode.MainSandbox },
    { pattern: `${JAY_QUERY_PREFIX}${RuntimeMode.WorkerTrusted}`, runtimeMode: RuntimeMode.WorkerTrusted },
    { pattern: `${JAY_QUERY_PREFIX}${RuntimeMode.WorkerSandbox}`, runtimeMode: RuntimeMode.WorkerSandbox },
];
 ...
 * 
 * @example
 * hasJayExtension('./file.jay-contract?jay-client', '.jay-html')
 * // false
 */
export function hasJayExtension(
    ...
    return (
        (basePath.endsWith(fullTsExtension) && basePath.length > fullTsExtension.length) ||
        (basePath.endsWith(fullTsxExtension) && basePath.length > fullTsxExtension.length)
    );
}
