...
/**
 * Scope state tracks the child element counter within a scope.
 * Each scope (page root, headless instance, forEach item) has its own counter.
 */
...
function walkChildren(
    ...
    for (const child of parent.childNodes) {
        ...
        // Uses the ref attribute (explicit or auto-generated by discoverHeadlessInstances)
        // to form the coordinate suffix: contractName:ref. No counter needed —
        // both discovery and assignCoordinates read the same ref attribute.
        // Use rawTagName to preserve original casing for coordinates.
        const rawTag = element.rawTagName;
        if (rawTag?.toLowerCase().startsWith('jay:')) {
            const contractName = rawTag.substring(4);
 ...
 * Uses the ref attribute for the coordinate suffix (not a counter index).
 */
...
function walkForEachChildren(
    ...
    for (const child of parent.childNodes) {
        ...
        // Headless instance inside forEach — uses ref attribute
        // Use rawTagName to preserve original casing for coordinates.
        const rawTag = element.rawTagName;
        if (rawTag?.toLowerCase().startsWith('jay:')) {
            const contractName = rawTag.substring(4);
