...
export function renderToStream(
    ...
    for (const vs1 of vs.products) {
        ...
        const vs_product_card0 = (vs as any).__headlessInstances?.[
            String(vs1._id) + ',product-card:AR0'
        ...
        if (vs_product_card0) {
            ...
            w('<span');
            w(' class="price"');
            w(' jay-coordinate="' + escapeAttr(String(vs1._id)) + '/product-card:AR0/0/1' + '">');
            w(escapeHtml(String(vs_product_card0.price)));
            w('</span>');
            w('<button');
            w(' jay-coordinate="' + escapeAttr(String(vs1._id)) + '/product-card:AR0/0/2' + '">');
            w('Add to Cart');
            w('</button>');
            w('</article>');
        }
