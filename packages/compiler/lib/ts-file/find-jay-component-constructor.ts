...
export const findJayComponentConstructor: SourceFileTransformer<
    FindJayComponentConstructorConfig
> = (
    factory: ts.NodeFactory,
    context: ts.TransformationContext,
    { callback, checker }: FindJayComponentConstructorConfig,
            ...
            if (ts.isNamespaceImport(namedBindings))
                ...
                namedBindings.elements.forEach((element) => {
                    ...
                    if (symbolType.flags && ts.TypeFlags.Object)
                        console.log(
                            'symbolType.objectFlags',
                            (symbolType as ts.ObjectType).objectFlags,
                        );
                });
