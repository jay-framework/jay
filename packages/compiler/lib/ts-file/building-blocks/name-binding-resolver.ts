import {
    ...
    FunctionLikeDeclarationBase, Identifier, isElementAccessExpression,
    isIdentifier,
    isObjectBindingPattern, isPropertyAccessExpression, isPropertyDeclaration, isStringLiteral,
    ParameterDeclaration, PropertyAccessExpression,
    PropertyName, VariableStatement
} from "typescript";
...
export function mkVariable(members: {
    ...
    return Object.fromEntries(
        Object.entries(members).filter(([key, value]) => value !== undefined)
    );
}

const getAccessedByProperty = (binding: Identifier, accessedFrom?: Variable, propertyName?: PropertyName) => {
    return accessedFrom?
        (propertyName?
            (isIdentifier(propertyName)?propertyName.text:undefined) :
            binding.text) :
        undefined;
}

export function tsBindingNameToVariable(binding: BindingName,
                                        accessedFrom?: Variable,
                                        assignedFrom?: Variable,
                                        propertyName?: PropertyName,
                                        root?: ParameterDeclaration): Variable[] {

    if (isIdentifier(binding)) {
        return [mkVariable({name: binding.text, accessedFrom, accessedByProperty: getAccessedByProperty(binding, accessedFrom, propertyName), assignedFrom, root})]
    }
    else if (isObjectBindingPattern(binding)) {
        let variable: Variable = mkVariable({accessedFrom, accessedByProperty: propertyName?(isIdentifier(propertyName)?propertyName.text:undefined):undefined, assignedFrom, root});
...
export class NameBindingResolver {
    ...
    addFunctionParams(functionDeclaration: FunctionLikeDeclarationBase) {
        functionDeclaration.parameters.map((param, index) => {
            let paramVariables = tsBindingNameToVariable(param.name, undefined, undefined, undefined, param)
    ...
    resolvePropertyAccessChain(expression: Expression): Variable {
        ...
        else if (isElementAccessExpression(expression) && isStringLiteral(expression.argumentExpression)) {
            const name = expression.argumentExpression.text;
            const identifiersFromObject = this.resolvePropertyAccessChain(expression.expression);
            return {accessedFrom: identifiersFromObject, accessedByProperty: name}
        }
        else if (isIdentifier(expression)) {
            return this.variables.has(expression.text)?
                this.variables.get(expression.text):
                {}
        } else {
            return {};
        }
    ...
    addVariableStatement(variableStatement: VariableStatement) {
        variableStatement.declarationList.declarations.forEach(declaration => {
            ...
            let declaredVariable = tsBindingNameToVariable(declaration.name, undefined, rightSide, undefined);
    ...
    resolveIdentifier(expression: Identifier): Variable {
        ...
        if (this.variables.has(expression.text))
            return this.variables.get(expression.text)
        else
            return {};
    }
}
...
export function flattenVariable(variable: Variable): FlattenedAccessChain {
    if (variable.assignedFrom)
        return flattenVariable(variable.assignedFrom)
    if (variable.accessedFrom) {
        let flattenedAccess = flattenVariable(variable.accessedFrom);
        return {path: [...flattenedAccess.path, variable.accessedByProperty], root: flattenedAccess.root}
    }
