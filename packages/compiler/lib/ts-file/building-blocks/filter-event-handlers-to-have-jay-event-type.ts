...
export function isFirstParamJayEvent(eventHandler: ts.FunctionLikeDeclarationBase, bindingResolver: SourceFileBindingResolver) {
    if (eventHandler.parameters.length > 0 &&
        ...
        if (ts.isTypeReferenceNode(eventHandler.parameters[0].type) &&
