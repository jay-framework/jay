...
type ComponentSecureFunctionsTransformerConfig = SourceFileTransformerContext & {patterns: string[]}

function parsePatterns(patterns: string[]) {
    patterns.forEach(pattern => {
        let patternSourceFile = ts.createSourceFile('dummy.ts', pattern, ts.ScriptTarget.Latest, false, ts.ScriptKind.TS)
        // find imports
        // find functions
...
function mkComponentSecureFunctionsTransformer(sftContext: ComponentSecureFunctionsTransformerConfig) {
...
export function componentSecureFunctionsTransformer(patterns: string[]): (
