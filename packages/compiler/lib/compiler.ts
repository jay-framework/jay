...
function renderInterface(aType: JayObjectType): string {
  ...
  genInterface += Object.keys(aType.props)
    .map((prop) => {
      ...
      } else if (childType instanceof JayAtomicType) return `  ${prop}: ${childType.name}`;
...
function renderAttributes(element: HTMLElement): string {
  ...
  Object.keys(attributes).forEach((attrName) => {
    if (attrName === 'if' || attrName === 'forEach' || attrName === 'trackBy') return;
    if (attrName === 'style')
      renderedAttributes.push(`style: {cssText: '${attributes[attrName]}'}`);
    else renderedAttributes.push(`${attrName}: '${attributes[attrName]}'`);
  });
...
function renderNode(
  ...
  function de(
    ...
    childLineBreaks: boolean
  ): RenderFragment {
    return new RenderFragment(
      `${firstLineIdent}de('${tagName}', ${attributes}, [${children.rendered}${
        childLineBreaks ? ident : ''
  ...
  function e(
    ...
    return new RenderFragment(
      `${firstLineIdent}e('${tagName}', ${attributes}, [${children.rendered}${
        childLineBreaks ? ident : ''
      }])`,
  ...
  function renderHtmlElement(htmlElement, newVariables: Variables) {
    let childNodes = node.childNodes.filter(
    ...
    else return e(htmlElement.rawTagName, attributes, childRenders, childLineBreaks);
  }

  function c(renderedCondition: RenderFragment, childElement: RenderFragment) {
    return new RenderFragment(
      `${firstLineIdent}c(${renderedCondition.rendered},\n${ident}${childElement.rendered}\n${firstLineIdent})`,
  ...
  function renderForEach(
    ...
    // todo item type
    return new RenderFragment(
      `${firstLineIdent}forEach(${renderedForEach.rendered}, (${collectionVariables.currentVar}: Item) => {
${ident}return ${childElement.rendered}}, '${trackBy}')`,
...
export function generateDefinitionFile(html: string): WithValidations<string> {
  ...
  return parsedFile.map((jayFile: JayFile) => {
    ...
    return [renderImports(Imports.for(Import.jayElement)), types, renderFunctionDecleration()].join(
      '\n\n'
    );
  });
}
...
export function generateRuntimeFile(html: string): WithValidations<string> {
  ...
  return parsedFile.map((jayFile: JayFile) => {
    ...
    return [
      renderImports(renderedImplementation.imports.plus(Import.element).plus(Import.jayElement)),
      types,
